...
Los campos de "info" que no aparezcan en la oferta déjalos como string vacío.`;
  ...
  try {
    ...
    if (data.stop_reason === 'max_tokens') {
      console.error('Respuesta cortada por max_tokens (posible búsqueda web extensa)');
      return res.status(500).json({ error: 'El análisis se cortó antes de terminar. Intenta de nuevo.' });
    }
